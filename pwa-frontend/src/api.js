...
async function apiFetch(path, opts = {}) {
  ...
  const timeoutId = setTimeout(() => controller.abort(), opts.timeout || 30000); // 30s default timeout
...
export async function getEta(stop_id) {
  return apiFetch(`/api/v1/eta?stop_id=${stop_id}`, { silent: true });
}

/** Get ML ETA prediction for all upcoming stops */
export async function getAllEtas() {
