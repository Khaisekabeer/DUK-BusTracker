...
import {
  getLatestGps, getTripState, getRouteHistory, getEta, getAllEtas, getRouteGeometry, getRouteSegment, getStops, getWsBusUrl
} from '../api';
...
export default function RouteView() {
  ...
  const [mlEtas, setMlEtas] = useState(null);
  ...
  useEffect(() => {
    // Small timeout ensures the DOM has updated and rendered the new id
            ...
            {stopsToShow.length === 0 ? (
              ...
              <div className={`ios-timeline ios-timeline--status-${globalDelayStatus}`}>
                {stopsToShow.map((stop, idx) => {
                  ...
                  // If bus is early (lateMins <= -1), assume it regulates schedule for stops AFTER the current one.
                  const effectiveLateMins = (lateMins != null && lateMins <= -1 && !isCurrent) ? 0 : lateMins;

                  const displayTime = isVisited
                    ? actualTime || scheduled
                    : effectiveLateMins != null
                      ? computeEstimatedTime(scheduled, effectiveLateMins) || scheduled
                      : scheduled;
                  ...
                  if (isSkipped) {
                    ...
                    if (mlEtas && mlEtas[stop.id] !== undefined && !isCurrent) {
                      ...
                      // Calculate the absolute delay in minutes based on ML ETA
                      if (scheduledMins) {
                        const mlPredictedDelay = Math.round(currentMins + mlEtas[stop.id] - scheduledMins);
                        
                        if (mlPredictedDelay > 1) {
                          delayType = 'late';
                          statusSubtext = `Predicted • Delayed ${mlPredictedDelay}m`;
                        } else if (mlPredictedDelay < -1) {
                          delayType = 'ahead';
                          statusSubtext = `Predicted • ${Math.abs(mlPredictedDelay)}m early`;
                        } else {
                          delayType = 'ontime';
                          statusSubtext = `Predicted • On time`;
                        }
                        usedMl = true;
                      }
                    }
                    ...
                    if (!usedMl) {
                      if (lateMins >= 1) {
                        ...
                        delayType = isCurrent ? 'ahead' : 'ontime';
                      } else {
                        delayType = 'ontime';
                      }
                          ...
                          <div className={`ios-track-line ${stopsToShow[idx + 1]?.name in visitedStops ? 'ios-track-line-visited' : ''}`}>
                            {isActive && anchorIdx === idx && stopProgress > 0 && !(stopsToShow[idx + 1]?.name in visitedStops) && (
                              <div style={{
                                ...
                                background: 'var(--timeline-track, #059669)'
                              }} />
                            )}
                          </div>
                        )}
