...
async def _mark_on_trip(
    ...
    trip.late_by_minutes = delay if delay > 0 else 0
        ...
        try:
            ...
            if candidates:
                ...
                if reached_stops:
                    ...
                    time_mins = now_ist.hour * 60 + now_ist.minute
                    sched_str = furthest_reached.morning_time if is_fwd else furthest_reached.evening_time
                    ...
                    await db.commit()
                    logger.info("[LIFECYCLE] Trip #%d reached stops: %s, updated delay: %s", trip.id, new_ids, trip.late_by_minutes)
                    ...
                    if manager:
                        await manager.broadcast({
                            ...
                            "status": trip.status,
                            "direction": trip.direction,
                            "late_by_minutes": trip.late_by_minutes
                        })
